...
void MuonCVXDDigitiser::LoadGeometry()
{
    ...
    if (!isVertex) {
        streamlog_out(DEBUG) << "Subdetector is: " << _subDetName << std::endl;
...
{ 
    ...
    if( STHcol != nullptr )
    {
       ...
       relCol->setFlag( lcFlag.getFlag()  ) ;
        ...
        streamlog_out( DEBUG ) << "Processing collection " << _colName  << " with " <<  nSimHits  << " hits ... " << std::endl ;
        for (int i=0; i < nSimHits; ++i)
        {
            ...
            streamlog_out( DEBUG ) << "Processing simHit #" << i << ", from layer=" << _currentLayer << ", module=" << _currentLadder << std::endl;
            streamlog_out (DEBUG) << "- EDep = " << simTrkHit->getEDep() *dd4hep::GeV / dd4hep::keV << " keV, path length = " << simTrkHit->getPathLength() * 1000. << " um" << std::endl;
            ...
            streamlog_out (DEBUG) << "- Position (mm) x,y,z,t = " << simTrkHit->getPosition()[0] << ", " << simTrkHit->getPosition()[1] << ", " << simTrkHit->getPosition()[2] << ", " << simTrkHit->getTime() << std::endl;
            streamlog_out (DEBUG) << "- Position r(mm),phi,theta = " << mcp_r << ", " << mcp_phi << ", " << mcp_theta << std::endl;
            streamlog_out (DEBUG) << "- MC particle pdg = ";
            EVENT::MCParticle *mcp = simTrkHit->getMCParticle();
            if (mcp) {
                streamlog_out (DEBUG) << simTrkHit->getMCParticle()->getPDG();
            } else {
                streamlog_out (DEBUG) << " N.A.";
            }
            streamlog_out (DEBUG) << std::endl;
            streamlog_out (DEBUG) << "- MC particle p (GeV) = " << std::sqrt(simTrkHit->getMomentum()[0]*simTrkHit->getMomentum()[0]+simTrkHit->getMomentum()[1]*simTrkHit->getMomentum()[1]+simTrkHit->getMomentum()[2]*simTrkHit->getMomentum()[2]) << std::endl;
            streamlog_out (DEBUG) << "- isSecondary = " << simTrkHit->isProducedBySecondary() << ", isOverlay = " << simTrkHit->isOverlay() << std::endl;
            streamlog_out (DEBUG) << "- Quality = " << simTrkHit->getQuality() << std::endl;
            ...
            // DEBUG DEBUGs to check if reconstruction went correctly
            // true global
            streamlog_out (DEBUG) << "- TRUE GLOBAL position (mm) x,y,z,t = " << simTrkHit->getPosition()[0] << ", " << simTrkHit->getPosition()[1] << ", " << simTrkHit->getPosition()[2] << ", " << simTrkHit->getTime() << std::endl;
            // true local (compare two verions)
            streamlog_out (DEBUG) << "- TRUE LOCAL position (localPos) (mm) x,y,z,t = " << localPos[0] << ", " << localPos[1] << ", " << localPos[2] << std::endl;
            // reco local 
            streamlog_out (DEBUG) << "- RECO LOCAL position (mm) x,y,z,t = " << recoHit->getPosition()[0] << ", " << recoHit->getPosition()[1] << ", " << recoHit->getPosition()[2] << std::endl;
            ...
            // reco global
            streamlog_out (DEBUG) << "- RECO GLOBAL position (mm) x,y,z,t = " << recoHit->getPosition()[0] << ", " << recoHit->getPosition()[1] << ", " << recoHit->getPosition()[2] << std::endl;
            ...
            streamlog_out (DEBUG) << "Reconstructed pixel cluster:" << std::endl;
            streamlog_out (DEBUG) << "- local position (x,y) = " << localPos[0] << "(Idx: " << localIdx[0] << "), " << localPos[1] << "(Idy: " << localIdx[1] << ")" << std::endl;
            streamlog_out( DEBUG ) << "(reco local) - (true local) (x,y,z): " << localPos[0] - _currentLocalPosition[0] << ", " << localPos[1] - _currentLocalPosition[1] << ", " << localPos[2] - _currentLocalPosition[2] << std::endl;
            streamlog_out (DEBUG) << "- global position (x,y,z, t) = " << recoHit->getPosition()[0] << ", " << recoHit->getPosition()[1] << ", " << recoHit->getPosition()[2] << ", " << recoHit->getTime() << std::endl;
            streamlog_out (DEBUG) << "- (reco global (x,y,z,t)) - (true global) = " << recoHit->getPosition()[0] - simTrkHit->getPosition()[0]<< ", " << recoHit->getPosition()[1] - simTrkHit->getPosition()[1] << ", " << recoHit->getPosition()[2] - simTrkHit->getPosition()[2]<< ", " << recoHit->getTime() - simTrkHit->getTime()<< std::endl;
            streamlog_out (DEBUG) << "- charge = " << recoHit->getEDep() << "(True: " << simTrkHit->getEDep() << ")"  << std::endl;
            streamlog_out (DEBUG) << "- incidence angles: theta = " << incidentTheta << ", phi = " << incidentPhi << std::endl;
            if (_produceFullPattern != 0)
            {
            ...
            streamlog_out (DEBUG) << "- number of pixels: " << recoHit->getRawHits().size() << std::endl;
            streamlog_out (DEBUG) << "- MC particle p=" << std::sqrt(simTrkHit->getMomentum()[0]*simTrkHit->getMomentum()[0]+simTrkHit->getMomentum()[1]*simTrkHit->getMomentum()[1]+simTrkHit->getMomentum()[2]*simTrkHit->getMomentum()[2]) << std::endl;
            streamlog_out (DEBUG) << "- isSecondary = " << simTrkHit->isProducedBySecondary() << ", isOverlay = " << simTrkHit->isOverlay() << std::endl;
            streamlog_out (DEBUG) << "- List of constituents (pixels/strips):" << std::endl;
            for (size_t iH = 0; iH < recoHit->rawHits().size(); ++iH) {
                SimTrackerHit *hit = dynamic_cast<SimTrackerHit*>(recoHit->rawHits().at(iH));
                streamlog_out (DEBUG) << "  - " << iH << ": Edep (e-) = " << hit->getEDep() << ", t (ns) =" << hit->getTime() << std::endl;
            }
            streamlog_out (DEBUG) << "--------------------------------" << std::endl;
                                          ...
                                          double *localDirection)
{
    ...
    streamlog_out( DEBUG ) << "Cell ID of Sim Hit: " << cellID0 << std::endl;
    SurfaceMap::const_iterator sI = _map->find( cellID0 ) ;
    const dd4hep::rec::ISurface* surf = sI->second ;
...
{
    streamlog_out( DEBUG ) << "Creating Ionization Points" << std::endl;
    double pos[3] = {0,0,0};
    double dir[3] = {0,0,0};
    double entry[3];
    double exit[3];
    ...
    streamlog_out( DEBUG ) << "local position: " << _currentLocalPosition[0] << ", " << _currentLocalPosition[1] << ", " << _currentLocalPosition[2] << std::endl;
    ...
    streamlog_out( DEBUG ) <<  "Track path length: " << trackLength << ", calculated dEmean * N_segment = " << dEmean << " * " << _numberOfSegments << " = " << dEmean*_numberOfSegments << std::endl;
    _eSum = 0.0;
    ...
    streamlog_out (DEBUG) << "Number of ionization points: " << _numberOfSegments << ", G4 EDep = "  << hcharge << std::endl;
    for (int i = 0; i < _numberOfSegments; ++i)
    {
        ...
        streamlog_out (DEBUG) << " " << i << ": z=" << z << ", eloss = " << de << "(total so far: " << _eSum << "), x=" << x << ", y=" << y << std::endl;
    }
    ...
    streamlog_out (DEBUG) << "Padding each segment charge (1/n^2 pdf) until total below " << _deltaEne << "e- threshold. New total energy: " << _eSum << std::endl;
    streamlog_out (DEBUG) << "List of ionization points:" << std::endl;
    for (int i =0; i < _numberOfSegments; ++i) {
        streamlog_out (DEBUG) << "- " << i << ": E=" << _ionisationPoints[i].eloss 
...
void MuonCVXDDigitiser::ProduceSignalPoints()
{
    ...
    streamlog_out (DEBUG) << "Creating signal points" << std::endl;
    for (int i = 0; i < _numberOfSegments; ++i)
    {
	    ...
	    streamlog_out (DEBUG) << "- " << i << ": charge=" << charge 
...
{  
    ...
    streamlog_out (DEBUG) << "Creating hits" << std::endl;
    for (int i=0; i<_numberOfSegments; ++i)
    {
        ...
        streamlog_out (DEBUG) << i << ": Pixel idx boundaries: ixLo=" << ixLo << ", iyLo=" << iyLo  
            <<  ", ixUp=" << ixUp << ", iyUp=" << iyUp << std::endl;
        for (int ix = ixLo; ix< ixUp + 1; ++ix)
        {   
            if ( (ix < 0) or (ix >= GetPixelsInaColumn()) ) {
                streamlog_out (DEBUG) << "Pixels in a column: " << GetPixelsInaColumn() << std::endl;
                streamlog_out (DEBUG) << "Skipping pixels with ix =" << ix << std::endl;
                continue;
            }
            for (int iy = iyLo; iy < iyUp + 1; ++iy)
            {
                if ( (iy < 0) or (iy >= GetPixelsInaRow()) ) {
                    streamlog_out (DEBUG) << "Pixels in a row: " << GetPixelsInaRow() << std::endl;
                     streamlog_out (DEBUG) << "Skipping pixels with iy =" << iy << std::endl;
                    continue;
                }
                ...
                streamlog_out (DEBUG) << "Integral x=" << integralX << ", Integral y=" << integralY << ", signal pt charge=" << spoint.charge << std::endl;
                ...
                if (item == hit_Dict.end())
                {
                    ...
                    streamlog_out (DEBUG) << "Created new pixel hit at idx=" << ix << ", idy=" << iy << ", charge=" << totCharge << std::endl;
                }
                else
                {
                    ...
                    streamlog_out (DEBUG) << "Updating pixel hit at idx=" << ix << ", idy=" << iy << ", total charge=" << edep << "(delta = " << totCharge << ")" << std::endl;
                }
            }
        }
    }
    streamlog_out (DEBUG) << "List of pixel hits created:" << std::endl; // still in local coords
    int idx=0;
    for(auto item : hit_Dict)
    {
        simTrkVec.push_back(item.second);       
        streamlog_out (DEBUG) << idx++ << ": x=" << item.second->getPosition()[0] << ", y=" << item.second->getPosition()[1] << ", z=" << item.second->getPosition()[2] << ", EDep = " << item.second->getEDep() << std::endl;
    }
}
/**
...
{
    streamlog_out (DEBUG) << "Adding Poisson smear to charge" << std::endl;
    for (int ihit = 0; ihit < int(simTrkVec.size()); ++ihit)
    {
        ...
        hit->setEDep(rng);
        streamlog_out (DEBUG) << ihit << ": x=" << hit->getPosition()[0] << ", y=" << hit->getPosition()[1] << ", z=" << hit->getPosition()[2] 
...
{
    streamlog_out (DEBUG) << "Adding FE noise smear to charge" << std::endl;
    for (int i = 0; i < (int)simTrkVec.size(); ++i)
    {
        ...
        streamlog_out (DEBUG) << i << ": x=" << hit->getPosition()[0] << ", y=" << hit->getPosition()[1] << ", z=" << hit->getPosition()[2] 
...
{
   streamlog_out (DEBUG) << "Applying threshold" << std::endl;
...
{
  streamlog_out (DEBUG) << "Charge discretization" << std::endl;
...
{
    streamlog_out (DEBUG) << "Adding resolution effect to timing measurements" << std::endl;
    for (int i = 0; i < (int)simTrkVec.size(); ++i)
    {
...
{
    streamlog_out (DEBUG) << "Time discretization" << std::endl;
...
TrackerHitPlaneImpl *MuonCVXDDigitiser::ReconstructTrackerHit(SimTrackerHitImplVec &simTrkVec)
{
    ...
    streamlog_out (DEBUG) << "Creating reconstructed cluster" << std::endl;
    ...
    for (size_t iHit=0; iHit < simTrkVec.size(); ++iHit)
    {
        ...
        time += hit->getTime();
        charge += hit->getEDep();
        streamlog_out (DEBUG) << iHit << ": Averaging position, x=" << hit->getPosition()[0] << ", y=" << hit->getPosition()[1] << ", weight(EDep)=" << hit->getEDep() << std::endl;
    ...
    streamlog_out(DEBUG) << "Edge sizes, minx, maxx, miny, maxy: " << edge_size_minx << ", " << edge_size_maxx << ", " << edge_size_miny << ", " << edge_size_maxy << std::endl;

    streamlog_out (DEBUG) << "Position: x = " << pos[0] << " + " << _layerHalfThickness[_currentLayer] * _tanLorentzAngleX << "(LA-correction)";
    pos[0] -= _layerHalfThickness[_currentLayer] * _tanLorentzAngleX;
    streamlog_out (DEBUG) << " = " << pos[0];

    streamlog_out (DEBUG) << "; y = " << pos[1] << " + " << _layerHalfThickness[_currentLayer] * _tanLorentzAngleY << "(LA-correction)";
    pos[1] -= _layerHalfThickness[_currentLayer] * _tanLorentzAngleY;
    streamlog_out (DEBUG) << " = " << pos[1];
    ...
    streamlog_out (DEBUG) << ", time (ns) = " << time << std::endl;
...
void MuonCVXDDigitiser::TransformToLab(const int cellID, const double *xLoc, double *xLab)
{
    ...
    streamlog_out( DEBUG ) << "Cell ID of Hit (used for transforming to lab coords)" << cellID << std::endl;
...
{
    streamlog_out(DEBUG) << "Number of layers: " << _numberOfLayers << std::endl;
    streamlog_out(DEBUG) << "Pixel size X: " << _pixelSizeX << std::endl;
    streamlog_out(DEBUG) << "Pixel size Y: " << _pixelSizeY << std::endl;
    streamlog_out(DEBUG) << "Electrons per KeV: " << _electronsPerKeV << std::endl;
    streamlog_out(DEBUG) << "Segment depth: " << _segmentDepth << std::endl;
    for (int i = 0; i < _numberOfLayers; ++i) 
    {
        streamlog_out(DEBUG) << "Layer " << i << std::endl;
        streamlog_out(DEBUG) << "  Number of ladders: " << _laddersInLayer[i] << std::endl;
        streamlog_out(DEBUG) << "  Radius: " << _layerRadius[i] << std::endl;
        streamlog_out(DEBUG) << "  Ladder length: " << _layerLadderLength[i] << std::endl;
        streamlog_out(DEBUG) << "  Ladder width: "<< _layerLadderWidth[i] << std::endl;
        streamlog_out(DEBUG) << "  Ladder half width: " << _layerLadderHalfWidth[i] << std::endl;
        streamlog_out(DEBUG) << "  Phi offset: " << _layerPhiOffset[i] << std::endl;
        streamlog_out(DEBUG) << "  Active Si offset: " << _layerActiveSiOffset[i] << std::endl;
        streamlog_out(DEBUG) << "  Half phi: " << _layerHalfPhi[i] << std::endl;
        streamlog_out(DEBUG) << "  Thickness: " << _layerThickness[i] << std::endl;
        streamlog_out(DEBUG) << "  Half thickness: " << _layerHalfThickness[i] << std::endl;
        //TODO: organize which get printed based on barrel/endcap
        streamlog_out(DEBUG) << "  Petal length: " << _layerPetalLength[i] << std::endl;
        streamlog_out(DEBUG) << "  Petal Inner Width: " << _layerPetalInnerWidth[i] << std::endl;
        streamlog_out(DEBUG) << "  Petal Outer Width: " << _layerPetalOuterWidth[i] << std::endl;
    }
}
//=============================================================================
